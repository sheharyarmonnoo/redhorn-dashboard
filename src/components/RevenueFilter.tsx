...
export default function RevenueFilter({ open, onClose, selectedUnits, onApply }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white border border-[#e4e4e7] rounded w-full max-w-[520px] mx-4 max-h-[80vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#e4e4e7]">
          <div>
            <p className="text-[14px] font-semibold text-[#18181b]">Filter Revenue by Unit</p>
        ...
        {/* Select All / None */}
        <div className="flex items-center gap-3 px-4 py-2 border-b border-[#f4f4f5]">
          <button
            onClick={selectAll}
            className={`text-[11px] font-medium px-2.5 py-1 rounded cursor-pointer transition-colors ${allSelected ? "bg-[#18181b] text-white" : "bg-[#f4f4f5] text-[#71717a] hover:bg-[#e4e4e7]"}`}
          >
            Select All
          </button>
          <button
            onClick={selectNone}
            className={`text-[11px] font-medium px-2.5 py-1 rounded cursor-pointer transition-colors ${noneSelected ? "bg-[#18181b] text-white" : "bg-[#f4f4f5] text-[#71717a] hover:bg-[#e4e4e7]"}`}
          >
            Clear All
          </button>
        </div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <button
                    ...
                    className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 cursor-pointer transition-colors ${
                      allIn ? "bg-[#18181b] border-[#18181b]" : someIn ? "bg-[#e4e4e7] border-[#a1a1aa]" : "border-[#d4d4d8]"
                    }`}
                  >
                    {allIn && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                    {!allIn && someIn && <span className="w-2 h-0.5 bg-[#71717a] rounded" />}
                  </button>
                  <p className="text-[11px] font-semibold text-[#18181b] uppercase tracking-wide">
                    Building {bldg}
                  </p>
                  ...
                  {units.map(t => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-2 py-1.5 rounded text-left cursor-pointer transition-colors ${
                        local.has(t.unit) ? "bg-[#f4f4f5]" : "hover:bg-[#fafafa]"
                      }`}
                    >
                      <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${
                        local.has(t.unit) ? "bg-[#18181b] border-[#18181b]" : "border-[#d4d4d8]"
                      }`}>
                        {local.has(t.unit) && <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                      </span>
                      <div className="min-w-0">
                        <p className="text-[11px] font-medium text-[#18181b]">{t.unit}</p>
                        <p className="text-[9px] text-[#a1a1aa] truncate">{t.tenant} — {formatCurrency(t.monthlyRent)}/mo</p>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-[#e4e4e7]">
          <p className="text-[11px] text-[#71717a]">{local.size} units · {formatCurrency(filteredRent)}/mo</p>
          <div className="flex items-center gap-2">
            <button onClick={onClose} className="text-[11px] font-medium px-3 py-1.5 text-[#71717a] hover:text-[#18181b] cursor-pointer">
              Cancel
            </button>
            <button onClick={apply} className="text-[11px] font-medium px-4 py-1.5 bg-[#18181b] text-white rounded hover:bg-[#27272a] transition-colors cursor-pointer">
              Apply Filter
            </button>
          </div>
        </div>
