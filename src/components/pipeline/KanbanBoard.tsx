  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2.5 px-0.5">
        ...
        <span className="text-[10px] font-medium text-[#71717a] bg-[#f4f4f5] px-1.5 py-0.5 rounded-md">
          {deals.length}
        </span>
      </div>
...
function SortableDealCard({ deal, onClick }: { deal: any; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "bg-white border border-[#e4e4e7] rounded-lg p-2.5 transition-all hover:border-[#a1a1aa] hover:shadow-sm",
    ...
    >
      <div className="flex gap-1.5">
        <div
          {...listeners}
          {...attributes}
          className="flex-shrink-0 pt-0.5 cursor-grab active:cursor-grabbing text-[#d4d4d8] hover:text-[#71717a]"
        >
          <GripVertical className="w-3.5 h-3.5" />
        </div>
        <div className="flex-1 min-w-0" onClick={onClick}>
          <div className="flex items-start justify-between mb-1">
            <h3 className="text-[13px] font-medium text-[#18181b] leading-snug line-clamp-2 pr-2 cursor-pointer hover:text-[#52525b]">
              {deal.name}
            </h3>
          </div>

          {brokerName && (
            <p className="text-[11px] font-medium text-[#71717a] mb-1 truncate">{brokerName}</p>
          )}

          {(deal.city || deal.state) && (
            <div className="flex items-center gap-1 text-[11px] text-[#a1a1aa] mb-2">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[11px] mb-2">
            <div className="flex justify-between">
              <span className="text-[#a1a1aa]">Ask</span>
              <span className="font-semibold text-[#27272a]">{deal.askingPrice ? formatCurrency(deal.askingPrice) : "—"}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <span className="text-[#a1a1aa]">$/SF</span>
              <span className="font-semibold text-[#27272a]">{deal.pricePerSF ? `$${deal.pricePerSF}` : "—"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#a1a1aa]">SF</span>
              <span className="font-semibold text-[#27272a]">{formatNumber(deal.sqft)}</span>
            </div>
          </div>

          <div className="flex items-center gap-1.5 flex-wrap">
            {deal.propertyType && (
              <span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-[#f4f4f5] text-[#52525b]">
                {deal.propertyType}
              </span>
            )}
            {deal.assignedTo && (
              <span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-[#eef2ff] text-[#4338ca]">
                {deal.assignedTo}
              </span>
            )}
          </div>
          ...
          {deal.updatedAt && (
            <p className="text-[10px] text-[#d4d4d8] mt-1.5">Updated {formatDate(deal.updatedAt)}</p>
          )}
        </div>
      </div>
    </div>
...
function DealCardOverlay({ deal }: { deal: any }) {
  return (
    <div className="bg-white border border-[#a1a1aa] rounded-lg p-2.5 shadow-lg rotate-1 w-[264px]">
      <h3 className="text-[13px] font-medium text-[#18181b] line-clamp-1">{deal.name}</h3>
      {deal.contacts?.[0]?.name && <p className="text-[11px] text-[#71717a]">{deal.contacts[0].name}</p>}
      <div className="flex gap-3 mt-1 text-[11px]">
        <span className="font-semibold text-[#27272a]">{deal.askingPrice ? formatCurrency(deal.askingPrice) : "—"}</span>
        <span className="text-[#a1a1aa]">{deal.capRate ? `${deal.capRate}%` : ""}</span>
      </div>
    </div>
  );
}
