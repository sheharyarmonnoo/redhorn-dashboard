...
function buildPrompt(propertyName: string, latestDate: string, latestPeriod: string | undefined, priorPeriod: string | undefined, latest: any[], priorSnapshot: any[] | null, receivables: any[], tenants: any[], priorAlerts: any[], falseFlags: any[]): string {
  ...
  return [
    ...
    `Each detail must be 2-4 short bullets — not a prose sentence. Use this pattern:`,
    ``,
    `- **${latestLabel.split(" ")[0]} CP**: **$X** vs **${priorLabel.split(" ")[0]} $Y** (**±N%** MoM)`,
    ...
    ``,
    `Rules for "detail":`,
    `- Bullets only, prefixed with "- ", each on its own line. No prose paragraphs.`,
    ...
    `Format the summary as 3 sections, each a **bold section heading** on its own line followed by 2-4 short bullet items. Use this exact pattern:`,
    ``,
    `**${latestLabel.split(" ")[0]} income movement**`,
    `- short scannable bullet 1`,
    `- short scannable bullet 2`,
    `- short scannable bullet 3`,
    ``,
