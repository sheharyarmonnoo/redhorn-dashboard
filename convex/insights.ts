...
function buildPrompt(propertyName: string, latestDate: string, latest: any[], priorSnapshot: any[] | null, receivables: any[], tenants: any[], priorAlerts: any[], falseFlags: any[]): string {
  ...
  return [
    ...
    ``,
    `IMPORTANT — summary formatting:`,
    `The "summary" field renders as markdown on the dashboard. Use:`,
    `- **bold** for tenant names, dollar figures, percentages, and units (e.g. **Eco-Comfort Foam Insulation** owes **$25,579** = **45%** of AR)`,
    `- Two newlines (\\n\\n) between paragraphs to break the wall of text into 2-4 short paragraphs`,
    `- One paragraph for income statement story, one for AR story, one for prior-issue continuation`,
    `- Lead each paragraph with a verb or topic word so the reader can scan`,
    ``,
